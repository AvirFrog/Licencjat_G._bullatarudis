...
def get_args():
    ...
    parser.add_argument("-l", "--leading", required=False, default="20", type=str, dest="leading",
                        help="Uzupełnij")#TODO: uzupełnij opis
    parser.add_argument("-t", "--trailing", required=False, default="30", type=str, dest="trailing",
                        help="Uzupełnij")#TODO: uzupełnij opis
...
def main(args):
    ...
    for file1, file2 in grouper(2, sorted(list(dir.iterdir()))):
        ...
        output = subprocess.run(["trimmomatic", "PE", "-threads", str(args.threads),
                                 ...
                                 f"ILLUMINACLIP:{str(adapters)}:{str(args.clip)}", f"LEADING:{str(args.leading)}",
                                 f"TRAILING:{str(args.trailing)}", f"MINLEN:{str(args.minlen)}"],
                                capture_output=True)
